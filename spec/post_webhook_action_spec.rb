describe Fastlane::Actions::PostWebhookAction do
  describe '#run' do
    it 'correctly sends a POST to the specified url' do
      # Since this is a single test we can avoid stubbing the HTTP
      # request and just perform it straight away.
      result = Fastlane::Actions::PostWebhookAction.run(url: "https://httpbin.org/post", api_key: "some_api_key", body: { "key" => "value" })
